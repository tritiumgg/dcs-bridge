...
| 1.4 | Windows cross-build in CI: `x86_64-pc-windows-msvc` through `cargo-xwin`, with `x86_64-pc-windows-gnu` as the documented fallback. The broker is never built with `panic = "abort"` (SPEC §14.2). | `lua-dcsbridge.dll` and `dcsb.exe` are produced from a Linux runner. |
...
| 6.4 | All thirteen `LIFECYCLE` topics, nine of them the hook driver's. | An `dcsb tail` logs the full sequence. The four sim-driver-emitted topics are the only ones missing with no sim driver. |
