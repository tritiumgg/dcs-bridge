...
| 1.4 | Windows cross-build in CI: `x86_64-pc-windows-msvc` through `cargo-xwin`, the only Windows target (DR-0003). The broker is never built with `panic = "abort"` (SPEC §14.2). | `lua-dcsbridge.dll` and `dcsb.exe` are produced from a Linux runner. |
